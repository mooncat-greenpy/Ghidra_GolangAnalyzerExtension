...
public class DatatypeDetailProvider extends ComponentProviderAdapter {

	private GolangAnalyzerExtensionPlugin gae_tool;
	private JPanel main_panel;

	public DatatypeDetailProvider(GolangAnalyzerExtensionPlugin tool, GolangDatatype go_datatype) {
		super(tool.getTool(), "DatatypeDetail", tool.getName());

		gae_tool=tool;
		main_panel=create_main_panel(go_datatype);

		setTitle(go_datatype.name);
		addToTool();
	}

	JPanel create_main_panel(GolangDatatype go_datatype) {
	...
	private JPanel create_info_panel(GolangDatatype go_datatype) {
		...
		JLabel name_panel=new JLabel(String.format("Name: %s", go_datatype.name));
		name_panel.setBorder(BorderFactory.createEmptyBorder(5,5,5,5));
		panel.add(name_panel);
		JLabel go_version_panel=new JLabel(String.format("Type: %s", go_datatype.kind.toString()));
		go_version_panel.setBorder(BorderFactory.createEmptyBorder(5,5,5,5));
		panel.add(go_version_panel);
		JLabel pointer_size_panel=new JLabel(String.format("Size: %d", go_datatype.size));
